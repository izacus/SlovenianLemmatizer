...
//-------------------------------------------------------------------------------------------
JoelRdr::~JoelRdr() {delete defaultRule;}

//-------------------------------------------------------------------------------------------
//Train a model from a file.
void JoelRdr::train(WordList::iterator &itStart, WordList::iterator &itEnd, int iVerbose, ostream &os)
{	int iWordNum = itStart.GetWordNum();
	...
	JoelTree			*r;
	defaultRule = new JoelTree(string(""), string(""), string(""), true);
	...
	if (iVerbose==1) cout << "JOEL: Starting train..." << endl;
...
double JoelRdr::lemmatize(WordList::iterator &itStart, WordList::iterator &itEnd, int iVerbose, ostream &os)
{	int iWordNum = itStart.GetWordNum();
	...
	double dAccurancy = (double) iTrue*100 / iWordNum;
...
JoelTree *JoelRdr::findRule(JoelTree *r, string &w, string &cond, string &conc, bool learn)
{
	vector<JoelTree *>::iterator		e;
	JoelTree						*er;
...
void JoelRdr::getTransfo(string &w, string &lem, string &cond, string &conc)
{
...
void JoelRdr::updateRule(JoelTree *r, string &w, string &cond, string &conc)
{
	if (r->conc == conc) {
		r->iWordPos++;
		r->words.push_back(w);
...
void JoelRdr::createNewExcep(JoelTree *r, string &w, string &cd, string &cc)
{
	JoelTree			*rule;
	dword	lc;
	...
	if (lc <= w.length())
	{	
		rule = new JoelTree(w, w.substr((w.length() - lc), lc), cc);
...
bool JoelRdr::diff(JoelTree *r, string cd)
{
...
bool JoelRdr::hasSuf(string &w, string &suf)
{
...
void JoelRdr::tokenize(const string& str, vector<string> &tokens, const string& delimiters)
{
...
//expressed in the following format: suf1 "\t" suf2 which means remove suffix1 
//from w and replace it by suffix2 to get lem.
void JoelRdr::applyRule(JoelTree *r, string &w, string &result)
{
	vector<string>	tokens;
	string			suf1;
	string			suf2;

	tokenize(r->conc, tokens, "\t");
...
void JoelRdr::PrintTree(ostream *fdRules, string tab, JoelTree *r, int depth)
{	if (r == NULL) r = defaultRule;

	vector<JoelTree *>::iterator		i;
...
void JoelRdr::PrintStat(ostream &os, int iType) const {
