...
******************************************************************************/
#include "../header/RdrStatistic.h"

//-------------------------------------------------------------------------------------------	
//main function for statistics
int RdrStatistic::StatAll(vector<string> vsInfile, string sOutfile, string sTextfile, bool bVerbose) {
	//timing
	Timing timeAll;
	...
	for (int i=0; i<vsInfile.size(); ++i) {
		...
		Statistics(wlst, ssOut, (bVerbose || sOutfile==""));
...
//-------------------------------------------------------------------------------------------	
int RdrStatistic::Statistics(WordList &wls, ostream &os, bool bVerbose){
	...
	for(WordList::iterator it = wls.begin(); it != wls.end(); ++it) {
		iAll ++;
...
//-------------------------------------------------------------------------------------------	
int RdrStatistic::CoveringStat(WordList &wls, string sTextfile, ostream &os, bool bVerbose){
...
//-------------------------------------------------------------------------------------------	
void RdrStatistic::CreateHeadLines(ostream &os, bool bCoverStat) {
...
map<string, int> RdrStatistic::GetWordsFromFile(string sFileName) {
	typedef map<string, int> mapsi;
	mapsi mWords;
